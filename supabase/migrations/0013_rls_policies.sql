...
CREATE POLICY rls_subscription_events_select ON subscription_events FOR SELECT TO authenticated
    USING (subscription_id IN (SELECT id FROM subscriptions WHERE organization_id = public.current_organization_id()));
...
CREATE POLICY rls_proposal_versions_select ON proposal_versions FOR SELECT TO authenticated
    USING (proposal_id IN (SELECT id FROM proposals WHERE organization_id = public.current_organization_id()));
...
CREATE POLICY rls_code_job_events_select ON code_job_events FOR SELECT TO authenticated
    USING (code_job_id IN (SELECT id FROM code_jobs WHERE organization_id = public.current_organization_id()));
...
CREATE POLICY rls_incident_signals_select ON incident_signals FOR SELECT TO authenticated
    USING (incident_id IN (SELECT id FROM incidents WHERE organization_id = public.current_organization_id()));
...
CREATE POLICY rls_job_events_select ON job_events FOR SELECT TO authenticated
    USING (job_id IN (SELECT id FROM jobs WHERE organization_id = public.current_organization_id()));
